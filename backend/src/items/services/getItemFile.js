...
async function getItemFile(ctx, { id, resolve } = {}) {
  ...
  if (resolve) {
    ...
    if ((!ctx.driveEnabled) || !item.driveFileId) {
      if (item.youtubeUrl) {
  ...
  if (item.driveFileId && ctx.driveEnabled) {
    try {
      ...
      // If Drive was enabled but the file isn't there, continue trying local storage.
    }
  }
  ...
  if ((!ctx.driveEnabled || !item.driveFileId) && item.youtubeUrl) {
    return {
      kind: 'redirect',
      url: item.youtubeUrl,
    };
