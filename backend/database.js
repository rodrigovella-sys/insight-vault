...
try {
  db.exec('CREATE INDEX IF NOT EXISTS idx_narrativeEntries_pillarId ON narrativeEntries (pillarId)');
  db.exec('CREATE INDEX IF NOT EXISTS idx_narrativeEntries_topicId ON narrativeEntries (topicId)');
  db.exec('CREATE INDEX IF NOT EXISTS idx_narrativeEntries_videoId ON narrativeEntries (videoId)');
  db.exec('CREATE INDEX IF NOT EXISTS idx_narrativeEntries_sourceSheet ON narrativeEntries (sourceSheet)');
} catch (_) {
  /* ignore */
}

// Best-effort one-time migration: if narrativeEntries were imported previously,
// copy them into items + itemTopics so the app uses a single unified table.
try {
  const narrativeCols = getTableColumns('narrativeEntries');
  if (narrativeCols.length) {
    db.exec(`
      INSERT OR IGNORE INTO items (
        id, filename, original, mimetype, size,
        text, summary, tags,
        skillValue, keyQuote, useCase, mediaType, youtubeUrl, timeRange,
        impact, contextText, linkStatus, relevance, exactTimestamp, seal,
        timestampStatus, valueRank, mediaRank, curation, playlist,
        thumbnail, videoId, macroPillar, subcategory, sourceBook, author,
        kindleSectionChapter, kindlePage, kindleLocation, sourceArtifact,
        originalSubtheme,
        pillarId, pillarName, topicId, topicName,
        status, createdAt, updatedAt
      )
      SELECT
        ne.id,
        'xlsx_' || ne.sourceSheet || '_' || ne.sourceRow,
        ne.falaChave,
        'text/narrative',
        0,
        ne.falaChave,
        ne.contexto,
        '[]',
        ne.valorHabilidade, ne.falaChave, ne.uso, ne.tipo, ne.linkYoutube, ne.minutos,
        ne.impacto, ne.contexto, ne.linkStatus, ne.relevancia, ne.timestampExato, ne.selo,
        ne.timestampStatus, ne.rankingValor, ne.rankingMidia, ne.curadoria, ne.playlist,
        ne.thumbnail, ne.videoId, ne.macroPilar, ne.subcategoria, ne.livroOrigem, ne.autor,
        ne.secaoCapituloKindle, ne.paginaKindle, ne.posicaoLocKindle, ne.arquivoOrigem,
        ne.subtemaOriginal,
        ne.pillarId,
        p.nameEn,
        ne.topicId,
        ...
        ne.createdAt,
        ne.updatedAt
      FROM narrativeEntries ne
      LEFT JOIN pillars p ON p.id = ne.pillarId
      LEFT JOIN topics  t ON t.id = ne.topicId
    `);
  }
} catch (_) {
  /* ignore */
...
try {
  const netCols = getTableColumns('narrativeEntryTopics');
  if (netCols.length) {
    db.exec(`
      INSERT OR IGNORE INTO itemTopics (itemId, topicId)
      SELECT entryId, topicId FROM narrativeEntryTopics
    `);
  }
} catch (_) {
  /* ignore */
